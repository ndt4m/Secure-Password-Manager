...
class Keychain {
   ...
   * Return Type: void
   */
  constructor(kvs, masterSalt, hmacSalt, aesSalt, masterKey, hmacKey, aesKey) {
    this.data = {
  ...
  static async init(password) {
    ...
    return new Keychain({}, encodeBuffer(stringToBuffer(masterSalt)), encodeBuffer(stringToBuffer(hmacSalt)), encodeBuffer(stringToBuffer(aesSalt)), masterKey, hmacKey, aesKey);
  }
  ...
  static async load(password, repr, trustedDataCheck) {
    ...
    return new Keychain(new_keychain.kvs, new_keychain.masterSalt, new_keychain.hmacSalt, new_keychain.aesSalt, masterKey, hmacKey, aesKey);
  };
  ...
  async get(name) {
    ...
    let isValid = await subtle.verify("HMAC", this.secrets.hmacKey, tag, key_val);
    if (!isValid)
    {
  ...
  */
  async set(name, value) {
    ...
    let tag = await subtle.sign("HMAC", this.secrets.hmacKey, key_val);
